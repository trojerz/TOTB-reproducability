...
for file in files_list:
	...
	list_img_names = list()
	list_gt_rect = list()
	init_rect = [int(s) for s in lines[0].replace(',', ' , ').split() if s.isdigit()]
	for i, line in enumerate(lines):
		list_gt = [int(s) for s in line.replace(',', ' , ').split() if s.isdigit()]
		list_gt_rect.append(list_gt)
		list_img_names.append(subdirectory + 'color/' + str(i+1).zfill(8) + '.jpg')
		
	data_set = {"video_dir": file, "img_names": list_img_names, "init_rect": init_rect, "gt_rect": list_gt_rect}
	data[file] = data_set

with open('TOTB.json', 'w') as outfile:
